...
namespace Base.UI {
    ...
    public class UIManagerFunctions : B_ManagerBase {
        ...
        public async void SetupSubframes() {
            ...
            foreach (var PickedInventorySlot in Subframes) await PickedInventorySlot.SetupFrame(this);
        ...
        public void MenuAdjustment() {
            if (Separated) {
                ...
                for (var i = 1; i < Subframes.Count + 1; i++) Subframes[i - 1].MoveUIRect(new Vector3(3000 * i, 0, 0));
        ...
        private void AddEmptyMenus() {
            for (var i = 0; i < Enum.GetValues(typeof(Enum_MenuTypes)).Length; i++) {
                var PickedObject = new GameObject();
                PickedObject.AddComponent<RectTransform>();
                PickedObject.transform.parent = MenuHolder;
                PickedObject.transform.localPosition = Vector3.zero;
                PickedObject.AddComponent(MenuType((Enum_MenuTypes)i).GetType());
                PickedObject.GetComponent<MenuUISubframe>().MenuType = (Enum_MenuTypes)i;
                PickedObject.GetComponent(MenuType(Enum_MenuTypes.Menu_GameOver).GetType());
            }
        }
