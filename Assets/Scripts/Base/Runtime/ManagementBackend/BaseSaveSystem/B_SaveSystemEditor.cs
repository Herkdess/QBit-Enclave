...
namespace Base {
    public class B_SaveSystemEditor {
        ...
        public void CreateNewSave() {
            var PickedObject = newBSaveObject;
            if (PickedObject.SaveName.MakeViable() == "") PickedObject.SaveName = "Empty_Save_Name";
            else PickedObject.SaveName = PickedObject.SaveName.MakeViable();
            PickedObject.name = PickedObject.SaveName;
            PickedObject.SaveThisData();
            PickedObject.Created = true;
            AssetDatabase.CreateAsset(PickedObject, "Assets/Resources/SaveAssets/" + PickedObject.SaveName + ".asset");
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            _saveObjects.Add(PickedObject);
            CreateEnums();
            newBSaveObject = ScriptableObject.CreateInstance<B_SaveObject>();
        }
        ...
        public void ClearAllSaves() {
            ...
            foreach (var PickedInventorySlot in AssetDatabase.FindAssets("", assetsPath)) {

                var path = AssetDatabase.GUIDToAssetPath(PickedInventorySlot);
