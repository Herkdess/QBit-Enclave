...
public class Sound {
    ...
    IEnumerator PlaySoundRoutine(int Loop, [CanBeNull]float delayBetween = 2f, [CanBeNull]float delay = 0f) {
        yield return new WaitForSeconds(delay);
        if (Loop <= 0) {
    ...
    IEnumerator InfiniteLoop([CanBeNull]float delayBetween = 2f) {
        while (true) {
            Source.Play();
            yield return new WaitForSeconds(SoundClip.length + delayBetween);
        }
